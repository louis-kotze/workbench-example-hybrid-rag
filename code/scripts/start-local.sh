...
CUDA_MEMORY_FRACTION=0.95 # adjust the percentage of the GPU being used. Don't go above 0.95

if [ "$2" = "none" ]
then
    text-generation-launcher --model-id $1 --cuda-memory-fraction $CUDA_MEMORY_FRACTION --max-input-length 8190 --max-total-tokens 8191 --port 9090 &
else
    text-generation-launcher --model-id $1 --cuda-memory-fraction $CUDA_MEMORY_FRACTION --max-input-length 8190 --max-total-tokens 8191 --quantize $2 --port 9090 &
fi
