...
class ChatClient:
    ...
    def search(
        ...
        with requests.post(url, headers=headers, json=data, timeout=300) as req:
            response = req.json()
            return typing.cast(
    ...
    def predict(
        ...
        with requests.post(url, stream=True, json=data, timeout=300) as req:
    ...
    def upload_documents(self, file_paths: typing.List[str]) -> None:
        ...
        for fpath in file_paths:
            ...
            _ = requests.post(
                url, headers=headers, files=files, verify=False, timeout=1800  # type: ignore [arg-type]
